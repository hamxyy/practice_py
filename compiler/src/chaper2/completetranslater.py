...
def match_statement(lex, node):
    return match_declaration(lex, node) or match_exp(lex, node)

def match_declaration(lex, node):
    type = lex.scan()
    id = lex.scan()
    equal = lex.scan()
...
def match_operator(op, e, node):
    ...
    if(match):
        op_node = SyntaxTreeNode(op)
        node.children.append(op_node)
        right_operand_node = SyntaxTreeNode(match.group(2))
        match_exp_result = match_exp(match.group(1), op_node)
        op_node.children.append(right_operand_node)
        if(match_exp_result):
            return True
    return False
